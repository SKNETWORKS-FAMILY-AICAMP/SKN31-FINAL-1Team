...
## overview(프로젝트 개요) 작성 규칙
- project.name을 문장에 그대로 끼워 넣지 마십시오.
  이름에 설명이 함께 적혀 있는 경우가 있어 문장이 어색해집니다.
  (예: "리테일링크(RetailLink) — 소상공인 재고/발주 관리 서비스")
  괄호와 부연 설명을 뺀 핵심 명칭만 사용하십시오.
- project.background가 비어 있으면 회의 목적을 그대로 옮기지 마십시오.
  requirements.functional을 참고해 "이 프로젝트가 무엇을 만드는지"를 씁니다.
  2번 문제 정의와 같은 문장이 되지 않도록 하십시오.   

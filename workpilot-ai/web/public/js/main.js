...
const modalRawLinkEl = document.getElementById("modal-raw-link");
...
function withPreservedInputs(fn) {
    const editable = Array.from(document.querySelectorAll("#panel textarea[id], #panel input[id], #panel select[id]"));
    const snapshots = editable.map((el) => ({
        id: el.id,
        value: el.value,
        selStart: "selectionStart" in el ? el.selectionStart : null,
        selEnd: "selectionEnd" in el ? el.selectionEnd : null,
...
function isHtmlDeliverable(url, language) {
...
async function openDeliverableModal(url, title, language) {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(`파일을 불러오지 못했습니다 (HTTP ${res.status})`);
        const text = await res.text();
        modalContentEl.textContent = text;
        if (isHtml)
            modalPreviewEl.srcdoc = text;
    }
    catch (err) {
...
document.body.addEventListener("click", (ev) => {
    ...
    if (action === "continue-suggestion" || action === "continue-custom") {
        ...
        const text = action === "continue-suggestion"
            ? (btn.dataset.text ?? "")
            : (document.getElementById("continue-text")?.value.trim() ?? "");
        if (!text)
            return toast("다음 지시 내용을 입력하세요.");
    ...
    if (action === "decompose") {
        ...
        const select = row?.querySelector("[data-select-member]");
        guarded(async () => {
        ...
        guarded(async () => {
            ...
            setState(await api.getProject(projectId));
            if (r.newAlerts.length > 0)
                toast(`지연 위험 ${r.newAlerts.length}건 감지됨.`);
            else
                toast("시간을 진행했습니다. 새 지연 알림은 없습니다.");
        });
    }
});
renderAll();

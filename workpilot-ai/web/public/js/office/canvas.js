import { drawActivityLabel, drawBookshelf, drawCharacter, drawDesk, drawPlant, drawSpeechBubble, preloadCharacterSprites, } from "./sprites.js";
...
export class OfficeCanvas {
    constructor(canvas) {
        ...
        this.members = [];
        this.prevTaskStatus = new Map();
        this.transientBubbles = new Map(); // memberId -> bubble
    ...
    layoutSeats() {
        ...
        const seats = [];
        if (lead)
            seats.push({ memberId: lead.id, x: w / 2, y: 110 });
    ...
    setState(members, tasks) {
        ...
        for (const m of members) {
            ...
            const waitingRecommendedFor = tasks.find((t) => t.status === "waiting_approval" && t.recommendation?.[0]?.memberId === m.id);
            if (delayed)
                states.set(m.id, { charState: "delayed", taskTitle: delayed.title });
            else if (active)
                states.set(m.id, { charState: "active", taskTitle: active.title });
            else if (waitingRecommendedFor)
                states.set(m.id, { charState: "waiting_approval", taskTitle: waitingRecommendedFor.title });
            else
                states.set(m.id, { charState: "idle" });
        }
    ...
    render() {
        ...
        for (const seat of orderedSeats) {
            ...
            if (!member)
                continue;
            const st = this.memberStates.get(seat.memberId) ?? { charState: "idle" };
            ...
            let bubbleKind = null;
            if (st.charState === "delayed")
                bubbleKind = "delay";
            else if (st.charState === "waiting_approval")
                bubbleKind = "waiting";
            else if (transient)
                bubbleKind = transient.kind;
            if (bubbleKind) {

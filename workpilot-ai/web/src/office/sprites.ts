...
const FRAME = { walk1: 0, walk2: 1, walk3: 2, type1: 3, type2: 4, read1: 5, read2: 6 } as const;

const imageCache: (HTMLImageElement | undefined)[] = [];

function getImage(palette: number): HTMLImageElement {
...
export function preloadCharacterSprites(): void {
  for (let i = 0; i < 6; i++) getImage(i);
}

export type CharState = "idle" | "active" | "delayed" | "waiting_approval";

export interface DrawOptions {
  x: number; // 캐릭터 중심 x (canvas px)
  y: number; // 발밑 기준 y (canvas px)
  palette: number;
  isLead?: boolean;
  state: CharState;
  bobPhase: number; // 애니메이션 위상(초 단위로 증가하는 값)
}

function frameForState(state: CharState, phase: number): number {
  switch (state) {
    case "active":
    case "delayed": {
...
export function drawCharacter(ctx: CanvasRenderingContext2D, opts: DrawOptions) {
...
const DECOR_SEED_ITEMS = ["plant", "mug", "books"] as const;
type DecorItem = (typeof DECOR_SEED_ITEMS)[number];

function decorFor(seed: number): DecorItem {
  return DECOR_SEED_ITEMS[seed % DECOR_SEED_ITEMS.length]!;
}

export interface DeskOptions {
  x: number; // 책상 중심 x
  y: number; // 책상 앞쪽 모서리 y (캐릭터 발밑과 맞춤)
  state: CharState;
  seed: number; // 팔레트 등 — 데스크 위 소품을 다양하게 고르는 용도
  glowPhase: number;
}
...
export function drawDesk(ctx: CanvasRenderingContext2D, opts: DeskOptions) {
  const { x, y } = opts;
...
export function drawPlant(ctx: CanvasRenderingContext2D, x: number, y: number, scale = 1) {
  ctx.save();
  ctx.translate(x, y);
...
export function drawBookshelf(ctx: CanvasRenderingContext2D, x: number, y: number, w = 64) {
  ctx.save();
  ctx.fillStyle = "rgba(0,0,0,0.2)";
...
export type BubbleKind = "waiting" | "done" | "delay";

export function drawSpeechBubble(ctx: CanvasRenderingContext2D, x: number, y: number, kind: BubbleKind) {
  const w = 34;
  const h = 22;
  const bx = x - w / 2;
  const by = y - h - 10;
...
export function drawActivityLabel(ctx: CanvasRenderingContext2D, x: number, y: number, text: string, state: CharState) {
  ...
  const h = 15;
  const color =
    state === "delayed" ? "#ef476f" : state === "active" ? "#06d6a0" : state === "waiting_approval" ? "#ffd166" : "#8a8aa0";
